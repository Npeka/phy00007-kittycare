...
void FirebaseManager::updateFoodAndDrinkStatus(const float &food, const float &drink) {
  ...
  snprintf(buf, sizeof(buf), "%04d-%02d-%02dT%02d:%02d:%02dZ",
           ...
           utcTime->tm_min,
           utcTime->tm_sec);
  Values::TimestampValue timeValue(buf);
  ...
  doc.add("time", Values::Value(timeValue));
  ...
  docs.createDocument(aClient, Firestore::Parent(projectId), collectionPath, DocumentMask(), doc, asyncCB,
                      FirebaseManager::getNameCreateDocCats().c_str());
}
...
String FirebaseManager::getNameCreateDocCats() const {
  return "firebase_create_doc_cats";
}
